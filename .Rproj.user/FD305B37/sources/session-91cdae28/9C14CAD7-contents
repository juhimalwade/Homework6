## HW5_testscript.R

library(testthat)

test_that("check validity method exists", {
  expect_false({
    validity_method <- getValidity(getClassDef("sparse_numeric"))
    is.null(validity_method)
  })
})

test_that("check validity method", {
  expect_true({
    x <- new("sparse_numeric",
             value = c(1, 2, 3, 1),
             pos = c(1L, 2L, 3L, 5L),
             length = 5L)
    validObject(x)
  })
})

test_that("check validity method 2", {
  expect_error({
    x <- new("sparse_numeric",
             value = c(1, 2, 3, 1),
             pos = c(1L, 2L, 3L, 5L),
             length = 5L)
    x@length <- 2L
    validObject(x)
  })
})

test_that("check coercion return class", {
  expect_s4_class({
    x <- as(c(0, 0, 0, 1, 2), "sparse_numeric")
  }, "sparse_numeric")
})

test_that("check for show method", {
  expect_no_error({
    getMethod("show", "sparse_numeric")
  })
})

test_that("check for plot method", {
  expect_no_error({
    getMethod("plot", c("sparse_numeric", "sparse_numeric"))
  })
})

test_that("check for + method", {
  expect_no_error({
    getMethod("+", c("sparse_numeric", "sparse_numeric"))
  })
})

test_that("check for - method", {
  expect_no_error({
    getMethod("-", c("sparse_numeric", "sparse_numeric"))
  })
})

test_that("check for * method", {
  expect_no_error({
    getMethod("*", c("sparse_numeric", "sparse_numeric"))
  })
})

test_that("sparse add generic", expect_true(isGeneric("sparse_add")))
test_that("sparse mult generic", expect_true(isGeneric("sparse_mult")))
test_that("sparse sub generic", expect_true(isGeneric("sparse_sub")))
test_that("sparse crossprod generic", expect_true(isGeneric("sparse_crossprod")))

test_that("sparse add formals", {
  expect_true(length(formals(sparse_add)) >= 2L)
})

test_that("sparse mult formals", {
  expect_true(length(formals(sparse_mult)) >= 2L)
})

test_that("sparse sub formals", {
  expect_true(length(formals(sparse_sub)) >= 2L)
})

test_that("sparse crossprod formals", {
  expect_true(length(formals(sparse_crossprod)) >= 2L)
})

test_that("check returned class for add", {
  expect_s4_class({
    x <- as(c(0, 0, 0, 1, 2), "sparse_numeric")
    y <- as(c(1, 1, 0, 0, 4), "sparse_numeric")
    sparse_add(x, y)
  }, "sparse_numeric")
})

test_that("sparse_add", {
  result <- as(c(1, 1, 0, 1, 6), "sparse_numeric")
  expect_equal({
    x <- as(c(0, 0, 0, 1, 2), "sparse_numeric")
    y <- as(c(1, 1, 0, 0, 4), "sparse_numeric")
    sparse_add(x, y)
  }, result)
})

test_that("sparse add dense", {
  result <- as(c(2, 4, 6, 10, 12), "sparse_numeric")
  expect_equal({
    x <- as(c(1, 3, 4, 1, 2), "sparse_numeric")
    y <- as(c(1, 1, 2, 9, 10), "sparse_numeric")
    sparse_add(x, y)
  }, result)
})

test_that("all zero wrong length", {
  expect_error({
    x <- as(rep(0, 10), "sparse_numeric")
    y <- as(rep(0, 9), "sparse_numeric")
    sparse_add(x, y)
  })
})

# Mean test
test_that("mean method exists", {
  expect_no_error(getMethod("mean", "sparse_numeric"))
})

test_that("mean computed correctly", {
  x <- as(c(0, 0, 3, 0, 2), "sparse_numeric")
  expect_equal(mean(x), (3 + 2) / 5)
})

test_that("mean of all-zero vector is zero", {
  x <- as(rep(0, 10), "sparse_numeric")
  expect_equal(mean(x), 0)
})

#Norm tests
test_that("norm generic exists", {
  expect_true(isGeneric("norm"))
})

test_that("norm method exists", {
  expect_no_error(getMethod("norm", "sparse_numeric"))
})

test_that("norm computed correctly", {
  x <- as(c(0, 3, 4), "sparse_numeric")
  expect_equal(norm(x), 5)
})

test_that("norm of zero vector is zero", {
  x <- as(rep(0, 8), "sparse_numeric")
  expect_equal(norm(x), 0)
})

#Standardize tests
test_that("standardize generic exists", {
  expect_true(isGeneric("standardize"))
})

test_that("standardize method exists", {
  expect_no_error(getMethod("standardize", "sparse_numeric"))
})

test_that("standardize returns correct class", {
  x <- as(c(1, 2, 3, 4, 5), "sparse_numeric")
  z <- standardize(x)
  expect_s4_class(z, "sparse_numeric")
})

test_that("standardized mean approx zero", {
  x <- as(c(1, 2, 3, 4, 5), "sparse_numeric")
  z <- standardize(x)
  expect_lt(abs(mean(z)), 1e-8)
})

test_that("standardized variance approx one", {
  x <- as(c(1, 2, 3, 4, 5), "sparse_numeric")
  z <- standardize(x)

  vals <- as.numeric(z)
  v <- mean((vals - mean(z))^2)

  expect_lt(abs(v - 1), 1e-6)
})

test_that("standardize errors when sd=0", {
  x <- as(rep(3, 10), "sparse_numeric")
  expect_error(standardize(x))
})

test_that("show method displays sparse_numeric object correctly", {
  x <- as(c(0, 1, 0, 3), "sparse_numeric")

  output <- capture.output(show(x))

  expect_true(any(grepl("length", output)))

  expect_true(any(grepl("Positions that are not zero", output)))

  expect_true(any(grepl("Values at those positions", output)))
})

test_that("plot method works without error", {
  x <- as(c(1, 0, 2, 0, 3), "sparse_numeric")
  y <- as(c(0, 5, 0, 6, 0), "sparse_numeric")

  expect_no_error(plot(x, y))
})

test_that("sparse_crossprod works correctly", {
  x <- as(c(1, 0, 2, 0), "sparse_numeric")
  y <- as(c(0, 3, 0, 4), "sparse_numeric")
  expect_true(isGeneric("sparse_crossprod"))

  expect_equal(sparse_crossprod(x, y), 0)  # No overlapping positions
  z <- as(c(1, 0, 2, 0), "sparse_numeric")
  w <- as(c(1, 0, 3, 0), "sparse_numeric")
  expect_equal(sparse_crossprod(z, w), 1*1 + 2*3)  # overlapping positions
})

test_that("+ operator works for sparse_numeric", {
  x <- as(c(1, 0, 2), "sparse_numeric")
  y <- as(c(0, 3, 1), "sparse_numeric")
  res1 <- sparse_add(x, y)
  res2 <- x + y
  expect_s4_class(res2, "sparse_numeric")
  expect_equal(res2, res1)
})

test_that("- operator works for sparse_numeric", {
  x <- as(c(5, 0, 2), "sparse_numeric")
  y <- as(c(2, 1, 0), "sparse_numeric")
  res1 <- sparse_sub(x, y)
  res2 <- x - y
  expect_s4_class(res2, "sparse_numeric")
  expect_equal(res2, res1)
})

test_that("* operator works for sparse_numeric", {
  x <- as(c(1, 0, 2), "sparse_numeric")
  y <- as(c(0, 3, 1), "sparse_numeric")
  res1 <- sparse_mult(x, y)
  res2 <- x * y
  expect_s4_class(res2, "sparse_numeric")
  expect_equal(res2, res1)
})

test_that("show method executes", {
  x <- as(c(0, 0, 1, 0), "sparse_numeric")
  expect_output(show(x), "Sparse numeric vector of length")
})


